...
from woolly.models.model import get_norm_layer, WyConv2d
from woolly.models.transform.spatial_transformer import SpatialTransformer
...
class WyBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, norm, ctype, stride=1):
        ...
        # Preprocess Block
        self.pconv1 = WyConv2d(
            in_planes, planes, kernel_size=3, strides=1, padding=1, ctype=ctype
        )
        self.pmp1 = nn.MaxPool2d(2)
        self.pbn1 = get_norm_layer(planes, norm)

        # ResBlock
        self.rconv1 = WyConv2d(
            planes, planes, kernel_size=3, strides=stride, padding=1, ctype=ctype
        )
        self.rbn1 = get_norm_layer(planes, norm)
        self.rconv2 = WyConv2d(
            planes, planes, kernel_size=3, strides=1, padding=1, ctype=ctype
        )
...
class CustomResNet(nn.Module):
    ...
    def _pre_layer(self, norm, ctype):
        layer = nn.Sequential(
            WyConv2d(
                3, self.in_planes, kernel_size=3, strides=1, padding=1, ctype=ctype
            ),
    ...
    def _res_layer(self, num_blocks, norm, ctype, stride):
        ...
        for stride in strides:
            layers.append(self.block(self.in_planes, out_planes, norm, ctype, stride))
        self.in_planes = out_planes
