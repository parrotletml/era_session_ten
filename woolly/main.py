...
from woolly.dataset.cifar.dataset import (
    get_cifar_loader,
    get_advance_cifar_loader,
)  # Load test and train loaders
from woolly.utils.transform import (
    BASE_PROFILE,
    get_transform,
)  # Get transforme functions
from woolly.utils.visualize import print_samples
...
def get_scheduler(epochs, lr, max_lr, mom, mom_max, optimizer, steps_per_epoch):
    ...
    mschedule = np.interp(
        np.arange(epochs + 1), [0, 7, epochs], [mom_max, mom, mom_max]
    )
